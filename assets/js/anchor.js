...
const joinRoom = async (e) => {
  ...
  // tracks 为房间内的远端资源，可直接订阅
	const { code, room, tracks } = await rtcClient.joinLivingRoom(roomId, window.RCRTC.RCLivingType.VIDEO);
...
const publish = async (localTracks) => {
  ...
  const { code, liveUrl } = await crtRoom.publish(localTracks);

	if (code === RCRTC.RCRTCCode.SUCCESS) {
    document.querySelector('#liveurl-input').value = liveUrl;
    /**
     * 播放资源
     */
...
const resetLeaveStyle = () => {
  ...
  document.querySelector('#liveurl-input').value = '';
};
